...
signed char CameraSerial_RxPacket[100];		//定义接收数据包数组，数据包格式"[MSG]"
uint8_t CameraSerial_RxFlag;			//定义蓝牙串口接收的标志位变量
...
void CameraSerial_Init(void)
{
	...
	RCC_APB2PeriphClockCmd(RCC_APB2Periph_GPIOC | RCC_APB2Periph_GPIOD, ENABLE);	//开启GPIOC的时钟
	...
	GPIO_Init(GPIOC, &GPIO_InitStructure);					//将PB10引脚初始化为复用推挽输出
	
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_IPU;
	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_2;
	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
	GPIO_Init(GPIOD, &GPIO_InitStructure);					//将PB11引脚初始化为上拉输入
...
void CameraSerial_SendArray(uint8_t *Array, uint16_t Length)
{
	...
	{
		CameraSerial_SendByte(Array[i]);		//依次调用CameraSerial_SendByte发送每个字节数据
	}
}
...
void CameraSerial_SendString(char *String)
{
	...
	{
		CameraSerial_SendByte(String[i]);		//依次调用CameraSerial_SendByte发送每个字节数据
	}
}
...
/**
  ...
  * 返 回 值：返回值等于X的Y次方
  */
uint32_t CameraSerial_Pow(uint32_t X, uint32_t Y)
{
...
void CameraSerial_SendNumber(int32_t Number, uint8_t Length)
{
	...
	if (Number < 0)
	{
		CameraSerial_SendByte('-');
		Number = -Number;
	}
	for (i = 0; i < Length; i ++)
	{
		CameraSerial_SendByte(Number / CameraSerial_Pow(10, Length - i - 1) % 10 + '0');
	}
}
...
void CameraSerial_Printf(char *format, ...)
{
	...
	va_end(arg);					//结束变量arg
	CameraSerial_SendString(String);	//蓝牙串口发送字符数组（字符串）
}
...
void UART5_IRQHandler(void)
{
	...
	{
		uint8_t RxData = USART_ReceiveData(UART5);			//读取数据寄存器，存放在接收的数据变量
		...
		if (RxState == 0)
		{
			if (RxData == '[' && CameraSerial_RxFlag == 0)		//如果数据确实是包头，并且上一个数据包已处理完毕
			{
				RxState = 1;			//置下一个状态
		...
		else if (RxState == 1)
		{
			...
			{
				RxState = 0;			//状态归0
				CameraSerial_RxPacket[pRxPacket] = '\0';			//将收到的字符数据包添加一个字符串结束标志
				CameraSerial_RxFlag = 1;	//接收数据包标志位置1，成功接收一个数据包
			}
			else						//接收到了正常的数据
			{
				CameraSerial_RxPacket[pRxPacket] = RxData;		//将数据存入数据包数组的指定位置
				pRxPacket ++;			//数据包的位置自增
			}
		}

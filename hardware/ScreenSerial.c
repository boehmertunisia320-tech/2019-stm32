...
signed char ScreenSerial_RxPacket[100];		//定义接收数据包数组，数据包格式"[MSG]"
uint8_t ScreenSerial_RxFlag;			//定义蓝牙串口接收的标志位变量
...
void ScreenSerial_Init(void)
{
	...
	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
	GPIO_Init(GPIOC, &GPIO_InitStructure);					//将PB11引脚初始化为上拉输入
...
void ScreenSerial_SendArray(uint8_t *Array, uint16_t Length)
{
	...
	{
		ScreenSerial_SendByte(Array[i]);		//依次调用ScreenSerial_SendByte发送每个字节数据
	}
}
...
void ScreenSerial_SendString(char *String)
{
	...
	{
		ScreenSerial_SendByte(String[i]);		//依次调用ScreenSerial_SendByte发送每个字节数据
	}
}
...
/**
  ...
  * 返 回 值：返回值等于X的Y次方
  */
uint32_t ScreenSerial_Pow(uint32_t X, uint32_t Y)
{
...
void ScreenSerial_SendNumber(int32_t Number, uint8_t Length)
{
	...
	if (Number < 0)
	{
		ScreenSerial_SendByte('-');
		Number = -Number;
	}
	for (i = 0; i < Length; i ++)
	{
		ScreenSerial_SendByte(Number / ScreenSerial_Pow(10, Length - i - 1) % 10 + '0');
	}
}
...
void ScreenSerial_Printf(char *format, ...)
{
	...
	va_end(arg);					//结束变量arg
	ScreenSerial_SendString(String);	//蓝牙串口发送字符数组（字符串）
}
...
void UART4_IRQHandler(void)
{
	...
	if (USART_GetITStatus(UART4, USART_IT_RXNE) == SET)	//判断是否是UART4的接收事件触发的中断
	{
		...
		if (RxState == 0)
		{
			if (RxData == '[' && ScreenSerial_RxFlag == 0)		//如果数据确实是包头，并且上一个数据包已处理完毕
			{
				RxState = 1;			//置下一个状态
		...
		else if (RxState == 1)
		{
			...
			{
				RxState = 0;			//状态归0
				ScreenSerial_RxPacket[pRxPacket] = '\0';			//将收到的字符数据包添加一个字符串结束标志
				ScreenSerial_RxFlag = 1;	//接收数据包标志位置1，成功接收一个数据包
			}
			else						//接收到了正常的数据
			{
				ScreenSerial_RxPacket[pRxPacket] = RxData;		//将数据存入数据包数组的指定位置
				pRxPacket ++;			//数据包的位置自增
			}
		}
